...
class ProposalTargetQuadrangleOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        #print 'type is',type(rois) numpy.ndarry
        '''
        rois_41 = []
        rois_14 = []
        rois_else = []
        for i in range(len(rois)):
            rois_x = float(rois[i][3]-rois[i][1])

            rois_y = float(rois[i][4]-rois[i][2])
            if rois_x == 0:
                rois_x =1
            rois_ratio = rois_y/rois_x
            if rois_ratio >= 4:
                rois_41.append(rois[i])
            elif rois_ratio <= 0.25:
                rois_14.append(rois[i])
            else:
                rois_else.append(rois[i])
        
        rois_41_c = np.array(rois_41)
        rois_14_c = np.array(rois_14)
        rois_else_c = np.array(rois_else)
        #print 'type rois is',type(rois_41_c) numpy.ndarry

        #if rois_41 == []:
        #    rois_41.append([0,0,0,10,10])
        
        rois_d = dict()
        rois_d.update({'rois_41':rois_41_c},{'rois_14':rois_14_c},{'rois_else':rois_else_c})
        #print 'rois_41 is',len(rois_41_c)
        #print 'rois_14 is',len(rois_14_c)
        #print 'rois_else is',len(rois_else_c)
        '''
        

        if DEBUG:
            print "labels=", gt_boxes.shape[0]
            print 'num fg: {}'.format((labels > 0).sum())
            # print 'num fg: {}'.format((labels > 0).sum()-gt_boxes.shape[0])
